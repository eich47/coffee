export class Cart {
  ...
  generateCart(content) {
    ...
    this.removeItem = this.createDomNode("span", "cart-checkout__close-icon");
    ...
    this.makeOrderButton = this.createDomNode("button", "cart-checkout__order");
  ...
  setContent(content) {
    const cartItems = content.map((item) => {
      ...
      cloneRemoveItem.addEventListener("click", (e) => {
        e.target.closest(`.${this.cartItemClass}`).remove();
      });
